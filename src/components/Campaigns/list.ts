const campaingList = [
  {
    name: "Mobília Solidária",
    id: 1,
    image: "/images/campaigns/logo-fecomerciors.webp",
    href: "/fecomerciors/",
    description:
      "A campanha <strong>Mobília Solidária</strong> é uma iniciativa do Fecomércio RS que visa arrecadar móveis e eletrodomésticos para empresas afetadas recentemente pelas enchentes. </br>Participe!"
  }
  // {
  //   name: "ReStart",
  //   id: 2,
  //   image: "/images/campaigns/logo-lacre.webp",
  //   href: "/manicures/",
  //   description:
  //     "A campanha <strong>ReStart</strong> é uma iniciativa da Lacre que visa ajudar manicures que foram atingidas pelas enchentes no RS."
  // }
];

export default campaingList;
